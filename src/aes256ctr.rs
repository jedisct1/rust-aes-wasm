...
//!
//! Provides encryption and decryption using AES-256 in CTR mode.
//!
//! ## Example
//! ```rust
//! use aes_wasm::aes256ctr::{encrypt, decrypt, Key, IV};
//! let key = Key::default();
//! let iv = IV::default();
//! let msg = b"hello";
...
/// Key type for AES-256-CTR (16 bytes).
pub type Key = [u8; KEY_LEN];
/// IV type for AES-256-CTR (16 bytes).
...
/// # Example
/// ```
/// use aes_wasm::aes256ctr::{encrypt, Key, IV};
/// let key = Key::default();
/// let iv = IV::default();
/// let msg = b"hello";
/// let ciphertext = encrypt(msg, &key, iv);
